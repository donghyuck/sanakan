...
- 가이드를 네 단계로 재구성했다. 설정 생성 Python 블록은 동일하며, 부분 JSON 예시와 그대로 유지할 값을 구분했다.
- 원격이 없는 임시 Git 저장소에서 가이드의 Python 설정 명령을 직접 실행했다.
  생성된 설정의 baseline/유효 기간 및 로컬 issue 식별자가 실제 config/check_issue 검사를 통과했다.
- 기존 CLI subprocess 회귀 `test_cli_with_fake_codex_process` 통과. 가짜 모델 프로세스 사용이며 실제 Codex 호출은 아니다.
- 실제 사용자 프로젝트·모델·GitLab은 실행하지 않았다. 전체 44개 테스트 결과는 아래 이전 버전 기록이다.
