...
from .common import (AUTOMATION, checkout, check_issue, config, digest, git, issue_iid,
                     locked, process, safe, save, task_dir, run_identity)
...
def verify(c, workspace, folder, manifest, stop_requested=lambda: False):
    from .execution import run_check
    # This tree is generated by the host immediately after applying the fixed patch.
    expected_tree = git(workspace, 'write-tree').decode().strip()
    expected_head = git(workspace, 'rev-parse', 'HEAD')
    def unchanged():
        if (git(workspace, 'rev-parse', 'HEAD') != expected_head or
                git(workspace, 'write-tree').decode().strip() != expected_tree or
                safe.changed_paths(Path(workspace), expected_tree)):
            raise ValueError('Verification changed the source tree; collect and review a new patch')
    unchanged()
    codes = []
    for index, check in enumerate(c['checks']):
        if stop_requested():
            raise ValueError('Stop requested before verification command')
        code, _ = run_check(c, check['argv'], workspace, folder / ('check-' + str(index) + '.log'))
        unchanged()
...
def run(config_path, issue_url, root, agents, provider=None, fixture=None, stop_requested=lambda: False):
    ...
    with locked(task_dir(root, c, iid)) as task:
        ...
        state = {'key': key, 'status': 'starting', 'issue_url': issue_url,
                 ...
                 'execution': c.get('execution', {'backend': 'local'})['backend']}
