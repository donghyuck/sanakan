...
def check_job_policy(c, effective):
    ...
    expected = dict(c, baseline=effective['baseline'])
    if effective != expected:
        raise ValueError('Project policy changed since job admission')
    ...
    for job in list(jobs(base).values()):
        ...
        if role == 'develop' and job['status'] == 'queued':
            try:
                ...
                result = run(settings, url, runs, agents or CodexAgents(c['timeout_seconds']),
...
def status(config_path, root):
    ...
    return {'services': services, 'jobs': jobs(base)}
...
def retry(config_path, root, iid):
    ...
    with locked(base / 'registry', blocking=True):
        ...
        if job['status'] in {'publish_failed', 'publish_retry', 'publishing'}:
