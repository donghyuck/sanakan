...
- studio-api 실제 적용에서 선택 BASELINE/PROJECT_MAP/.agent 문서 부재로 차단되는 프롬프트 충돌을 수정했다.
- Worker가 implemented로 코드 변경을 인계하면 호스트가 독립 검증하고 읽기 전용 완료 보고/리뷰를 수행한다.
- Worker의 미검증 보고를 보존하며 gate는 여전히 completed 및 실제 검증·리뷰 성공을 요구한다.
- 실제 studio-api snapshot에서 테스트 3개 보강, 총 10개 통과 및 독립 리뷰 pass/ready를 확인했다.
- source 프로젝트에는 검증된 테스트 파일과 적용 지침만 반영했다. 원격 게시/자동 감시/운영 배포는 하지 않았다.

## 1.6.1 — 2026-09-22

- studio-api의 로컬/원격 기준, 지침 충돌, 검증 명령과 프로젝트 형식을 직접 대조한 적용 가이드를 추가했다.
- 별도 복제본의 문서 메타데이터 단위 테스트 7개 통과 및 baseline bundle 크기를 확인했다.
- 테스트 파일 하나를 대상으로 한 로컬 설정 초안을 추가했다. 만료/계정/path 값은 자동 실행되지 않도록 명시했다.
- 대상 studio-api의 정책·애플리케이션 코드·원격 PR은 변경하지 않았다. 실행기 코드 변경은 없다.
