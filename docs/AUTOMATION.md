...
Sanakan 1.4.0은 GitLab 이슈를 주기적으로 확인해 개발·검증·독립 리뷰를 수행하고,
...
python3 -m sanakan start --role develop \
  --config /protected/project.json --runs /var/lib/sanakan
```
...
동일한 보호된 설정·소스 경로·공유 run 저장소가 필요하다. 현재는 단일 호스트의 별도 계정/환경을 전제로 한다.
공유 디렉터리 권한은 운영자가 준비하고, 개발 코드가 정책·검증 증거를 변경하지 못하도록 격리한다.
...
주요 작업 상태는 queued → developing → ready → publishing → published다.
...
중복 방지·수정 반복·재시도·중지·재시작·형식 검사를 확인한다. 실제 모델/GitLab/알림/CI는 별도 실증이 필요하다.
